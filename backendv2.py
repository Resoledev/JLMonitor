...
def save_state(products, current_product_ids, state_file):
    ...
    for product in products:
        ...
        if product_id in previous_state:
            old_price = previous_state[product_id].get("latest_price")
            current_price = product["current_price"]
            
            # If price has dropped
            if old_price is not None and current_price is not None and current_price < old_price:
                new_state[product_id]["recently_reduced"] = True
                new_state[product_id]["reduced_timestamp"] = current_time
            # If price hasn't changed, keep existing recently_reduced status if it's recent (e.g. < 48h)
            elif previous_state[product_id].get("recently_reduced"):
                reduced_ts = previous_state[product_id].get("reduced_timestamp")
                if reduced_ts:
                    try:
                        reduced_dt = datetime.fromisoformat(reduced_ts)
                        if datetime.now() - reduced_dt < timedelta(hours=168):  # 1 week
                            new_state[product_id]["recently_reduced"] = True
                            new_state[product_id]["reduced_timestamp"] = reduced_ts
                    except ValueError:
...
def send_webhook(products, previous_state, category_name, global_seen: Set[str] = None):
    ...
    for i, (product, event_type, price_diff, direction) in enumerate(items_to_report, 1):
        print(f"Sending webhook {i}/{len(items_to_report)}: {product['name'][:50]}...")
...
def fetch_product_info(url, counter, total, category_name):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            logging.error(f"SSL error on {url} (attempt {attempt+1}): {ssl_err}")
            if attempt == max_attempts - 1:
                send_error_webhook(f"SSL error on {url}: {ssl_err}")
                return []
            time.sleep(random.uniform(5, 10))
